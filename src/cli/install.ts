...
export async function installCommand(
  ...
  const vendorId = resolveVendorId(specVendor ?? opts.vendor, config);
  ...
  if (fs.existsSync(finalDir)) {
    if (!opts.force) {
      ...
      log.info(`run: jvm use ${artifact.javaVersion.major}`);
      return;
    }
  ...
  await withLock(async () => {
    ...
    try {
      ...
      const normalized = normalizeExtracted(tmp, platform);
  ...
  log.info(`switch to it: jvm use ${artifact.javaVersion.major}`);
}
