...
describe('registry', () => {
  ...
  it('major match picks highest across vendors', () => {
    mkJdk('temurin-21.0.5+11');
    mkJdk('zulu-21.0.12.1');
    expect(findInstalled('21').dirPath.endsWith('zulu-21.0.12.1')).toBe(true);
  });

  it('vendor prefix match', () => {
    mkJdk('temurin-21.0.5+11');
    mkJdk('zulu-21.0.12.1');
    expect(findInstalled('temurin-21').dirPath.endsWith('temurin-21.0.5+11')).toBe(true);
  });

  it('full prefix match ignores build', () => {
    mkJdk('temurin-21.0.5+11');
    expect(findInstalled('21.0.5').dirPath.endsWith('temurin-21.0.5+11')).toBe(true);
  });

  it('lts match', () => {
    mkJdk('temurin-21.0.5+11');
    mkJdk('temurin-22.0.1+2');
    expect(findInstalled('lts').version.major).toBe(21);
  });
